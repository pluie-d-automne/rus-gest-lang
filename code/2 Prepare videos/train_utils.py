...
class VideoDatasetBig(Dataset):
    def __init__(self, video_labels, video_dir, IMG_SIZE, max_frame_count, classes, face_detection_model, ds_type = "test"):
        super(VideoDatasetBig, self).__init__()
        self.video_labels = video_labels
        self.video_dir = video_dir
        self.IMG_SIZE = IMG_SIZE
    ...
    def load_video(self, path, begin, end, max_frames=0, label=None):
        blank_frame = np.zeros((self.IMG_SIZE,self.IMG_SIZE,3), np.uint8)
        cap = cv2.VideoCapture(path)
        frames = []
        min_y, max_y, min_x, max_x = 0, 0, 0, 0
        frame_index=begin+1
        ...
        if len(frames)==0:
          ...
          return None
        totalt_frame_cnt = len(frames)
        if totalt_frame_cnt < self.max_frame_count:
          frames.extend([blank_frame]*(self.max_frame_count-totalt_frame_cnt))
        else:
          all_idxs = random.sample(range(totalt_frame_cnt), k=self.max_frame_count)
          frames = [frames[i] for i in sorted(all_idxs)]
        return torch.from_numpy(np.array(frames))
